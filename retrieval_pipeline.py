...
def main() -> None:
    ...
    parser.add_argument(
        "--query", default="In which episode do Jim and Pam kiss?", help="Search query"
    )
    parser.add_argument("--k", type=int, default=12, help="Top-k results")

    parser.add_argument(
        "--query-expansion", action="store_true", help="Enable query expansion + RRF fusion"
    )
    ...
    parser.add_argument(
        "--expand-cache",
        default="experiments/cache/query_expansion_cache.json",
        help="Expansion cache",
    )
    ...
    expand_cfg = QueryExpansionConfig(
        ...
        cache_path=(
            None
            if not str(args.expand_cache).strip()
            else __import__("pathlib").Path(args.expand_cache)
        ),
    )
    ...
    for q in expanded:
        ...
        per_query[q] = [
            (d, float(s))
            for d, s in db.similarity_search_with_relevance_scores(q, k=int(args.k_per_query))
        ]
