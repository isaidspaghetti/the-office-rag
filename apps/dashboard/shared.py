...
def _pick_latest_phase_summary_json() -> Optional[Path]:
    ...
    if ctxfix:
        ...
        ctxfix_llm_phase = [
            p for p in ctxfix if "_llm" in p.name.lower() and "by_step" not in p.name.lower()
        ]
        if ctxfix_llm_phase:
...
def _mmr_pairs(
    ...
    *,
    ...
    """
    effective_fetch_k = (
        int(fetch_k) if isinstance(fetch_k, int) and fetch_k > 0 else int(max(int(k) * 4, 20))
    )
    ...
    for r in rows:
        ...
        out.append(
            PhaseRow(
                ...
                best_avg_overall=(
                    int(r["best_avg_overall"]) if r.get("best_avg_overall") is not None else None
                ),
...
def _load_run_rows(*, runs_dir: Path, scored_dir: Path, require_llm: bool) -> List[RunRow]:
    ...
    for rf in _iter_run_files(runs_dir):
        ...
        created_at = (
            _parse_utc((run or {}).get("created_at_utc")) if isinstance(run, dict) else None
        )
